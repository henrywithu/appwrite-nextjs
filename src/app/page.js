...
export default function Home() {
  ...
  async function uploadCsvData() {
    ...
    reader.onload = async (event) => {
      ...
      // Simple parser: one document per line.
      const documents = csvText.split('\\n')
        .filter(row => row.trim() !== '')
        .map(row => ({ content: row.trim().replace(/^"|"$/g, '') }));
      ...
      try {
        ...
        const promises = documents.map(doc => databases.createDocument(DATABASE_ID, COLLECTION_ID, ID.unique(), doc));

        await Promise.all(promises);
        alert(`${documents.length} documents created successfully from CSV!`);
      } catch (error) {
      ...
      <section className="mt-4 flex flex-col items-center gap-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="block w-full cursor-pointer text-sm text-slate-500 file:mr-4 file:rounded-full file:border-0 file:bg-violet-50 file:py-2 file:px-4 file:text-sm file:font-semibold file:text-violet-700 hover:file:bg-violet-100"
          />
          <button onClick={uploadCsvData} className="cursor-pointer rounded-md bg-[#FD366E] px-4 py-2 text-white" disabled={!csvFile}>Upload CSV</button>
        </div>
      </section>
